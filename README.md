...
- **单个 ipk** — 传一个文件，装一次，卸载无残留
- **纯静态 musl 二进制**（~511KB）— 不依赖固件 libc，OpenWrt 21.02 ~ 24.10 都能跑
...
从 [Releases](../../releases) 下载对应架构的包，传到路由器安装。

### 支持的架构

| OpenWrt 架构 | 典型设备 |
|---|---|
| `aarch64_cortex-a53` | CMCC RAX3000M、NanoPi R2S/R4S、树莓派 4 |
| `aarch64_cortex-a72` | NanoPi R5S/R6S、树莓派 5 |
| `x86_64` | 软路由、虚拟机、OpenWrt x86 |
| `arm_cortex-a7_neon-vfpv4` | IPQ40xx 等 ARMv7 路由器 |

### OpenWrt 21.02 ~ 23.05（opkg）

```sh
scp campus-auth-guardian_*_aarch64_cortex-a53.ipk root@192.168.1.1:/tmp/
...
```sh
scp campus-auth-guardian_*_aarch64_cortex-a53.apk root@192.168.1.1:/tmp/
ssh root@192.168.1.1 "apk add --allow-untrusted /tmp/campus-auth-guardian_*.apk"
```

装完自动启用，去 **LuCI → 服务 → 校园网认证 → 设置** 填配置。
...
sh scripts/build-all.sh --no-build   # 跳过编译，用已有二进制
```
